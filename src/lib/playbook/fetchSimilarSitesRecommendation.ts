...
function buildSystemInstruction(): string {
  return [
    ...
    "이 앱은 ‘프롬프트 작업 결과’ 플레이북이지만, 추천 목록은 **실제로 비슷한 일을 하는 제품·서비스** 위주여야 합니다.",
    "쇼핑·결제·배송 추적·예약·고객센터·헬프센터·티켓/이슈·팀 협업·문서 허브처럼 **AI와 무관해도 쓰이는 운영 사이트**를 우선하고, 채널(고객 대면·티켓·문서 등)에 맞는 UX·운영 사례를 골라 주세요.",
    "범용 챗 LLM·프롬프트 마켓·코딩 에이전트 제품 등 **AI 전용 사이트는 0~2개로 제한**하고, 나머지는 위와 같은 실제 비즈니스/소비자 기능 사이트로 채우세요. 전부 AI 도구면 잘못된 응답입니다.",
    "개발·문서 맥락이면 공식 문서·스펙은 일부 포함해도 되지만, 목록 전반이 벤더 문서뿐이면 안 됩니다.",
    "",
    "규칙:",
    "- 한국어만.",
...
function buildUserContent(picked: (string | null)[]): string {
  ...
  const head =
    "아래는 프롬프트 플레이북 선택값입니다. 채널·리스크·검증 방식에 맞춰, **같은 역할을 하는 실제 서비스(쇼핑·결제·CS·티켓 등)**를 우선 추천하고 AI 도구만 나열하지 마세요.";
